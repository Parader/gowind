...
export const ModalOverlay = (props: AriaModalOverlayProps) => {
    return (
        <AriaModalOverlay
            ...
            className={(state) =>
                cx(
                    "fixed inset-0 z-[1100] flex min-h-dvh w-full items-end justify-center overflow-y-auto bg-overlay/70 px-4 pt-4 pb-[clamp(16px,8vh,64px)] outline-hidden backdrop-blur-[6px] sm:items-center sm:justify-center sm:p-8",
...
interface ModalHeaderProps extends ComponentPropsWithRef<"header"> {
    onClose?: () => void;
}

export const ModalHeader = ({ className, children, onClose, ...props }: ModalHeaderProps) => {
    return (
        <header {...props} className={cx("flex w-full items-start justify-between gap-4", className)}>
            {children}
            {onClose && (
                <CloseButton aria-label="Close" size="sm" className="shrink-0" onPress={onClose} />
            )}
        </header>
    );
};
